...
class SegDataset(Dataset):
    def __init__(self, root='data/pc', meta='data/train_raw.txt', labelweight = 'data/labelweights.npz', frames_per_clip=3, num_points=16384, train=True):
        ...
        self.train = train
        self.root = root
        self.embedding2 = root.replace('prNpz', 'embedding2')
        ...
        with open(meta, 'r') as f:
            for line in f:
                ...
                fn_embedding = os.path.join(self.embedding2, sequence_name + '-' + str(frame_id) + '.npz')
                data = np.load(fn)
                data_embedding = np.load(fn_embedding)
                pr_emb1 = data_embedding['pr_emb1']
                pr_emb2 = data_embedding['pr_emb2']   # 128dim
                ...
                self.data[sequence_name + '-' + str(frame_id)] = (pc, rgb, semantic, center, primitive)
                self.primitive_feature[sequence_name + '-' + str(frame_id)] = (pr_emb1, pr_emb2)
    ...
    def read_training_data_point(self, index):
        ...
        for diff in range(0, self.frames_per_clip):
            ...
            if key in self.data:
                pc, rgb, semantic, center, primitive = self.data[key]
                most_recent_success = frame_id - diff
            else:
                pc, rgb, semantic, center, primitive = self.data[sequence_name + '-' + str(most_recent_success)]
        ...
        for diff in range(0, 5):
            key = sequence_name + '-' + str(frame_id-diff)
            if key in self.primitive_feature:
                pc1, pc2 = self.primitive_feature[key]
                most_recent_success = frame_id - diff
            else:
                pc1, pc2 = self.primitive_feature[sequence_name + '-' + str(most_recent_success)]
        ...
        primitive = np.stack(primitives, axis=0)

        pc1 = np.stack(pc1s, axis=0)
        pc2 = np.stack(pc2s, axis=0)
        return pc, rgb, semantic, center_0, primitive, pc1, pc2


    def half_crop_w_context(self, half, context, pc, rgb, semantic, center, primitive):
        ...
        primitive_half_w_context = [primitive[f, s] for f, s in enumerate(sample_indicies_half_w_context)]
        if half == 0:
        ...
        return pc_half_w_context, rgb_half_w_context, semantic_half_w_context, loss_masks, valid_pred_idx_in_full, primitive_half_w_context
    ...
    def choice_to_num_points(self, pc, rgb, label, primitive, loss_mask, valid_pred_idx_in_full):
        ...
        for f in range(self.frames_per_clip):
            ...
            primitive[f] = primitive[f][shuffle_idx]
        ...
        primitive = np.stack(primitive, axis=0)
        loss_mask = np.stack(loss_mask, axis=0)
        valid_pred_idx_in_full = np.stack(valid_pred_idx_in_full, axis=0)

        return pc, rgb, label, primitive, loss_mask, valid_pred_idx_in_full

    def __getitem__(self, index):
        context = 1.

        pc, rgb, semantic, center, primitive, pc1_embedding, pc2_embedding = self.read_training_data_point(index)

        half = 0
        pc1, rgb1, semantic1, mask1, valid_pred_idx_in_full1, primitive1 = self.half_crop_w_context(half, context, pc, rgb, semantic, center, primitive)
        label1, mask1 = self.mask_and_label_conversion(semantic1, mask1)
        pc1, rgb1, label1, primitive1, mask1, valid_pred_idx_in_full1 = self.choice_to_num_points(pc1, rgb1, label1, primitive1, mask1, valid_pred_idx_in_full1)

        half = 1
        pc2, rgb2, semantic2, mask2, valid_pred_idx_in_full2, primitive2 = self.half_crop_w_context(half, context, pc, rgb, semantic, center, primitive)
        label2, mask2 = self.mask_and_label_conversion(semantic2, mask2)
        pc2, rgb2, label2, primitive2, mask2, valid_pred_idx_in_full2 = self.choice_to_num_points(pc2, rgb2, label2, primitive2, mask2, valid_pred_idx_in_full2)
        ...
        clips = primitive.shape[0]
        for clip in range(clips):
            index1 = np.argsort(primitive1[clip])
            pc1[clip] = pc1[clip][index1]
            rgb1[clip] = rgb1[clip][index1]
            label1[clip] = label1[clip][index1]
            mask1[clip] = mask1[clip][index1]

            index2 = np.argsort(primitive2[clip])
            pc2[clip] = pc2[clip][index2]
            rgb2[clip] = rgb2[clip][index2]
            label2[clip] = label2[clip][index2]
            mask2[clip] = mask2[clip][index2]

        rgb1 = np.swapaxes(rgb1, 1, 2)
        rgb2 = np.swapaxes(rgb2, 1, 2)


        return pc1.astype(np.float32), rgb1.astype(np.float32), label1.astype(np.int64), mask1.astype(np.float32), pc2.astype(np.float32), rgb2.astype(np.float32), label2.astype(np.int64),  mask2.astype(np.float32), pc1_embedding, pc2_embedding
