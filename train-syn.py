...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch, print_freq):
    ...
    for pc1, rgb1, label1, mask1, pc2, rgb2, label2, mask2, pc1_embedding, pc2_embedding in metric_logger.log_every(data_loader, print_freq, header):
        start_time = time.time()

        pc1, rgb1, label1, mask1, pc1_embedding = pc1.to(device), rgb1.to(device), label1.to(device),  mask1.to(device), pc1_embedding.to(device)
        pc2, rgb2, label2, mask2, pc2_embedding = pc2.to(device), rgb2.to(device), label2.to(device),  mask2.to(device), pc2_embedding.to(device)
 

        output1 = model(pc1, rgb1, pc1_embedding).transpose(1, 2)
        ...
        output2 = model(pc2, rgb2, pc2_embedding).transpose(1, 2)
...
def evaluate(model, criterion, data_loader, device, print_freq):
    ...
    with torch.no_grad():
        for pc1, rgb1, label1, mask1, pc2, rgb2, label2, mask2, pc1_embedding, pc2_embedding in metric_logger.log_every(data_loader, print_freq, header):
            pc1_embedding = pc1_embedding.to(device)
            pc2_embedding = pc2_embedding.to(device)
...
def parse_args():
    ...
    parser.add_argument('--data-train', default='', help='meta list for training')
    parser.add_argument('--data-eval', default='', help='meta list for test')
    ...
    # input
    parser.add_argument('--clip-len', default=1, type=int, metavar='N', help='number of frames per clip')
    parser.add_argument('--num-points', default=16000, type=int, metavar='N', help='number of points per frame')
    # P4D
